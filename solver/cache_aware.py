...
def generate_schedule(
    ...
    partition = partition_contiguous(analysis, group_count, problem=3)
    return schedule_partition(analysis, partition, ncores, problem=3, config=config)


def candidate_counts(analysis: GraphAnalysis, ncores: int) -> list[int]:
    return candidate_group_counts(len(analysis.eligible_ops), ncores, problem=3)